...
public class CustomDriver {
    ...
    public boolean isElementPresent(String locator, String info) {
        ...
        System.out.println("Email count is::" + size);
        if (size > 0) {
            return true;
        } else {
            return false;
    ...
    public void selectFromListofEmail(String locator, String info, String subject) {
        List<WebElement> elementList = getElementList(locator, info);
        int size = elementList.size();
       
        for(WebElement ele : elementList) {
			if(ele.getText().equals(subject)) {
				ele.click();
				break;
			}
		}
	...
	public void scrollIntoViewJS(String locator, String info) {
		WebElement element = getElement(locator, info);
		js = (JavascriptExecutor) driver;
